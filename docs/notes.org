...
*** Regression testing notes
    ...
    http://www.warpx.org/wiki/index.php?title=WarpXRegOverview

    The procedure to create a new regression test is simple. 

    First, select an input file for the test you wish to run. Do not
    select input files which take too long to run. At most the
    simulation should be run in a minute or so, preferably in a few
    seconds. If you have a simulation which takes longer, then reduce
    the end time or the resolution.

    Next, put the input file into the

     warpx/regtests/tests

    directory. This directory already has a bunch of sub-directories
    inside it. If your test falls into one of the categories, add it
    to that sub-directory. If not, you will need to add a new
    sub-directory, put the input file in it. Do not forget to svn-add
    you new file and sub-directory!

    Next, open the file

      warpx/regtests/xregtests.inp

    This is the file which describes how your test should be
    run. Notice that this file looks like a usual WarpX input
    file. Each sub-directory of the warpx/regtests/tests directory has 
