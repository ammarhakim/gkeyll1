...
** Week of Feb 16th
   ...
   Added getRankOfInterface(ifc) method. This method takes the name of
   the interface, for example "CE" or "CS" etc and returns the rank in
   the component which should be used for communication. For this,
   another method getParentRankForChildRank(nm, rk) was added. This
   method returns the rank in the parent's communicator corresponding
   to the rank in the child communicator.

   Hence, in an updater, when one needs to communicate data between
   two children component, one needs to do:

     int cr1 = child1->getRankOfInterface(ifc);
     int cr2 = child1->getRankOfInterface(ifc);

     int pr1 = this->getParentRankForChildRank(child1Name, cr1);
     int pr2 = this->getParentRankForChildRank(child2Name, cr2);

   Then one can communicate using pr1 and pr2.

   The above does not work as-is. Unfortunately one can not query a
   child for the interface rank because it may not be valid on all
   processors. For example, consider the core-edge case. When we are
   on the core PEs we need to know which PE of the edge to send the
   data to. Unfortunately we can't query the edge component for its
   getRankOfInterface() as the edge component is not valid on core
   PEs.

   Hence, the map of the IFC -> rank needs to be stored in the parent
   in a manner that this map is valid on all processors. This involves
   adding the following method to each component:

     std::vector<std::string> getNamesOfInterfaces() const;

   This will return to the parent what interfaces a component
   supports. Then in the FcComponent::setAttrib() the parent uses this
   to query the rank for that interface, broadcasts it to all
   processors in its communicator and stores it. Then, the updaters
   can use instead:

     int getRankOfInterface(childName, ifcName);

   rather than directly querying the child component's
   getRankOfInterface().
