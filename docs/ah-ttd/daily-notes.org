...
#+END_EXAMPLE or a more complex data-structure. The key could be the
  ...
  register. When the ObjCreator class moduleRegister() method is
  called it should create add the base class exposed methods and
  register the methods in a loop.
