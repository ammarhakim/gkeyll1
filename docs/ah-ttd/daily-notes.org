...
* December 26th
  ...
  Found bug in WavePropagationUpdater. Works for 1D advection
  problem. Need to verify second order convergence and then test with
  Euler equations.
  ...
  was to simply use the 1D index to set the pointers].
...
#+END_EXAMPLE
  ...
  What are features of a self-describing block? Call them "traits"
  following Enthought.
  ...
  - Each trait can take values in a range: unbound, one from a set, or
    inside a range (for numbers).

  - Number of entries in a vector trait can be from a range.
