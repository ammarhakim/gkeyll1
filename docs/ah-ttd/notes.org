...
** Initializing Lucee with LUA tables.
   ...
   This needs to be done NOW. The basic idea is to have the top-level
   Lucee executable read a Lua script file and initialize and run the
   simulation. For example, one can imagine
#+BEGIN_EXAMPLE
   rteSlab = RteHomogenousSlab {
     L = 300,
     N = 650,
     mmin = 0,
     mmax = 1,
