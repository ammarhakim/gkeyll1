...
* June 18th
  ...
  builds with bilder and is installed in the usual nice June.

** TODO Complete fixed-potential Vlasov journal note

   For this, need to implement moment computing updater [DONE]. Several new
   matrices are needed to perform the integration. [DONE]

   Also, a volume intergration updater is needed. [DONE]

   At this point this is the top priority for this week. Need to
   finish this note as all the needed code is completed and all that
   needs to be done is pick some good problems and write up the
   results.
...
** A Frankenstein Monster?
   ...
   project look like a Frankenstein monster. However, there is no
   other option.
...
** TODO General nodal-DG updater

   Add an updater to solve general hyperbolic conservation laws using
   a nodal DG scheme.

** TODO Complete proof of DG energy conservation

   This requires some thought on how the discontinuity in d(phi)/dx
   should be taken into account.

** TODO Registration of objects
...
** TODO Some nodal updaters only work for scalar fields

   Fix this. For eample, the new IntegrateNodalField only works with
   scalar fields. This is not usually a problem as aliases can alway
   be created, but could cause problems on an unsuspecting user. Fix,
   or barf.

** TODO DOCUMENTATION!

   This is a perpetually underlooked todo. I need to document each
   updater, datastructure and grid object. Each should be in its own
   RST file rather than a single file for all updaters etc. This makes
   searching and indexing easier.

   Also, need to add an overview of the whole system, explaning how to
   write a Lua script to drive Gkeyll.

   Finally, need to clean up the C++ API documentation. This is an
   enormous task and just gets more unmanegable as time goes on, more
   classes are added, methods are added etc. etc. etc.

** TODO Improve HDF5 output

   Allow appending data to the same file. Also somehow one needs to
   use better names for the output groups and dataspaces.

** TODO Enable initializing fields from HDF5 files

   So restart can be enabled.

** TODO Why does Lua script not have access to command line options?

   Please fix this, should be easy (I think).
