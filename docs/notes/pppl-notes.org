...
* March 20th
  ...
  In this last month I have presented at SIAM and started work on a
  paper decribing the inconsistency of standard DG schemes for
  diffusion. In the last few days have also spent time thinking about
  the leap to 3D/4D/5D kinetic equations.

  Need to move out all the Gkeyll documents (tech notes and papers) to
  a new repo. Manual could still reside in the main gkeyll repo, but
  other things need to move out.

* Febuary 27th

  How can one off-load as much of the work to libraries, so that
  Gkeyll can become as small as possible. All the libraries needed
  (Blitz++, Eigen, Boost, etc) are already linked to Gkeyll. Need to
  make best use of them, and cut down on total C++ LOCs.

* Febuary 23rd

  More work on diffusion equation schemes with DG. The initial
  condition issue is now worked out, at least for sin(k*x)
  initialization. However, this does not seem to improve the
  convergence of the scheme.

  Need to write this up next week, while in Boston, based on the
  initial draft Greg wrote.

* Febuary 20th

  Did initial tests of recovery based DG scheme (RDG) and tested it on
  diffusion problem. We see very high order of convergence of the
  solutions, much higher than the corresponding order DG scheme.

  Trying to work out recovery based DG scheme. Turns out that doing a
  van Leer style recovery (i.e. best L2 fit in each cell) does not
  work as that just gives the standard DG scheme back. Doing a best L2
  fit across three cells, with the constraint that the edge values be
  upwind, gives a scheme, which, apparently, is not
  superconvergent. This is an odd result, which I do not know how to
  explain. Perhaps there is an error in the script.

  Last weekend I also added some particle classes to Gkeyll. These are
  not complete yet, as I have not yet figured out how to do the move
  of particles between cells. When this is complete, I want to PIC
  simulations of reconnection and do side-by-side comparison of
  two-fluid 5/10 moment and PIC.

* Febuary 13th

  Completed (with Eric Shi) notes on various diffusion schemes with
  DG. Turns out that the best scheme to use is van Leer's recovery
  based DG scheme (RDG). These names, "recovery" and "reconstruction"
  are horrible. Did a bunch of von Neumann analysis, results of which
  were surprizing. Eric will now implement a modal RDG in Gkell and
  test.

* Febuary 8th

  Completed tests for the general decomposition with nodal DG
  algorithm. Seems to work and now all of Gkeyll explicit updaters are
  parallel. Next, to think of the updaters that use petsc.
