...
** Track down "problem" with copyAllDataFromField() method	     :GKEYLL:

   Not sure if this is a problem, but it needs to be tracked down as a
   regression test is failing.

** Make regression tests take extra command-line parameters 	     :GKEYLL:

   This will allow passing extra parameters to gkeyll when running the
   tests. In the long run it might be better to have a script to run
   each test so this arbitrary addition of stuff is not needed.

** Test 1D and 3D element quadratures for LagrangeTensorBasisCalc    :GKEYLL:

   Also add more tests for 2D.
...
** Write source integrator for nodal DG scheme			     :GKEYLL:
   ...
   So source terms can be included in DG updates. One way to do this
   is treat the sources in a seperate updater just like the auxillary
   variables are treated. This would remove the need to create aliases
   of DG fields.
...
** Write HyperEqn for collisions with LB operator 	             :GKEYLL:
...
** Write a ExplicitFiveMomentSrc updater			     :GKEYLL:

** Add collisional terms to momentum equations			     :GKEYLL:
...
** Write JE on collision operator				     :GKEYLL:

   Test and document use of collision operator class.
...
** Split tests/slvr directory into multiple directories		      :INFRA:

   Slvr directory is getting very crowded.
...
** Enable initializing fields from HDF5 files			      :INFRA:
   ...
   So restart can be enabled. For this, pass command line parameters
   to the Lua script so it can restart instead of calling initialize
   method/updater.
...
** Parallel HDF5 unit test fails				      :INFRA:
   
   Somehow, reading a vector attribute fails on my Mac. It works on
   portal, however.
