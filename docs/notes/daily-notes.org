...
* January 5th 2012
  ...
  Now in Princeton temp housing. Apartment is old but okay. Needs a
  through cleaning.

  Completed the decomposition neighbor calculation code. Code seems to
  work. A general issue is how to test methods and data-structures
  that are internal to a class. Most unit tests are "black-box" and
  can only work with the public interface of a class. Then the
  question: how to test internals of a class?

  Need to write an LDEVP on parallel field messaging, multi-region
  data-structures and a tree-like data-structure for storing arbitrary
  hierarchical data.

* December 31st

  Added a new directory in docs called ldevp which will contain Lucee
  development proposals. For now onwards I want to write a LDEVP
  before doing the implementation.

  Working on DecompRegion class. Added a LDEVP on a proposed
  implementation.

  Need to next add a multi-region class that will allow representing a
  multi-block grid.

  Need to add a general tree structure that allows storing
  hierarchical data in key-value pairs. The supported values should be
  int, double, string, vectors of these and 1, 2 and 3d
  arrays. Potentially, this could also store parallel arrays, i.e
  arrays that live on different processors.
