...
\begin{frame}{Passive advection is a good prototype to study DG
    schemes}
  Consider the 1D passive advection equation on $\Omega\in [L,R]$
  \begin{align*}
  ...
  First step is to discretize the domain into elements $\Omega_j$,
  and pick a finite-dimensional function space to represent the
  solution.
  \begin{align*}
    \mathcal{V}^k_m(K) \equiv \{w: w|_{K_j} \in P^k(K_j) \} \cap C^m
  \end{align*}
  where $P^k(K_j)$ is the space of polynomials of order at most $k$
  on the element $K_j$.
\end{frame}
% ----------------------------------------------------------------
...
\begin{frame}{Discrete problem can be stated as finding the
  ...
  \mathcal{V}^k_{-1}$ such that for all smooth $w$ we have
  \begin{align*}
    \int_{K_j} w\left(
      \pfrac{f_h}{t} 
      + \lambda \pfrac{f_h}{x}
  ...
  \begin{align*}
    \int_{K_j} w \pfrac{f_h}{t}\thinspace dx
    +
    \lambda \hat{f}_h
    -
  ...
  \emph{numerical} flux on $\partial K_j$.
