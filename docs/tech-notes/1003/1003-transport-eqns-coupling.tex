...
we must set $\gvec{\Gamma}=0$ at $x=0$. At $x=x_m$ one can either
...
the interval $[t_n,t_{n+1}]$ will give schemes with different
implicitness. The numerical flux function must satisfy the consistency
condition
\begin{align}
...
$\mvec{F}_{1/2}=0$. If the flux is specified at $x=x_m$, then this can
be enforced using $\mvec{F}_{N+1/2}=\mvec{F}_m(t)$. For the second
...
if we set $\mvec{Q}_{N+1} = 2\mvec{Q}_m(t) - \mvec{Q}_N$, we will
enforce $\mvec{Q}(x_m,t) = \mvec{Q}_m(t)$ to $\textrm{O}(\Delta
x^2)$. Using this in the definition of the numerical flux, we get the
flux at the last edge as
\begin{align}
  \mvec{F}_{N+1/2} = \mvec{H}(\mvec{Q}_N, 2\mvec{Q}_m(t) - \mvec{Q}_N).
\end{align}
