/**
 * @file	LcImplicitTwentyMomentSrcUpdater.cpp
 *
...
// set ids for module system
  template <> const char *ImplicitTwentyMomentSrcUpdater<1>::id = "ImplicitTwentyMomentSrc1D";
  template <> const char *ImplicitTwentyMomentSrcUpdater<2>::id = "ImplicitTwentyMomentSrc2D";
  template <> const char *ImplicitTwentyMomentSrcUpdater<3>::id = "ImplicitTwentyMomentSrc3D";

  template <unsigned NDIM>
  void
  ImplicitTwentyMomentSrcUpdater<NDIM>::readInput(Lucee::LuaTable& tbl)
  {
...
// charge on each fluid species
    ...
    {
      Lucee::Except lce("ImplicitTwentyMomentSrcUpdater::readInput: 'charge' table should have exactly ");
...
// mass of each fluid species
    ...
    {
      Lucee::Except lce("ImplicitTwentyMomentSrcUpdater::readInput: 'mass' table should have exactly ");
...
// linear solver type
    ...
    if (tbl.hasString("linearSolver"))
    {
      ...
      {
        Lucee::Except lce("ImplicitTwentyMomentSrcUpdater::readInput: 'linearSolver' must be ");
...
// flag to indicate if there is a static field
  ...
  template <unsigned NDIM>
  void
  ImplicitTwentyMomentSrcUpdater<NDIM>::initialize()
  {
  ...
  template <unsigned NDIM>
  Lucee::UpdaterStatus
  ImplicitTwentyMomentSrcUpdater<NDIM>::update(double t)
  {
...
// create LHS matrix
        ...
        // create RHS matrix
        double v1 = fPtr[RHOUX]/fPtr[RHO];
        double v2 = fPtr[RHOUY]/fPtr[RHO];
        double v3 = fPtr[RHOUZ]/fPtr[RHO];

        qRhs[0] = fPtr[Q111] - 3*v1*fPtr[P11] + 2*fPtr[RHO]*v1*v1*v1;
        qRhs[1] = fPtr[Q112] - 2*v1*fPtr[P12] - v2*fPtr[P11] + 2*fPtr[RHO]*v1*v1*v2;
        qRhs[2] = fPtr[Q113] - 2*v1*fPtr[P13] - v3*fPtr[P11] + 2*fPtr[RHO]*v1*v1*v3;
        qRhs[3] = fPtr[Q122] - v1*fPtr[P22] - 2*v2*fPtr[P12] + 2*fPtr[RHO]*v1*v2*v2;
        qRhs[4] = fPtr[Q123] - v1*fPtr[P23] - v2*fPtr[P13] - v3*fPtr[P12] + 2*fPtr[RHO]*v1*v2*v3;
        qRhs[5] = fPtr[Q133] - v1*fPtr[P33] - 2*v3*fPtr[P13] + 2*fPtr[RHO]*v1*v3*v3;
        qRhs[6] = fPtr[Q222] - 3*v2*fPtr[P22] + 2*fPtr[RHO]*v2*v2*v2;
        qRhs[7] = fPtr[Q223] - 2*v2*fPtr[P23] - v3*fPtr[P22] + 2*fPtr[RHO]*v2*v2*v3;
        qRhs[8] = fPtr[Q233] - v2*fPtr[P33] - 2*v3*fPtr[P23] + 2*fPtr[RHO]*v2*v3*v3;
        qRhs[9] = fPtr[Q333] - 3*v3*fPtr[P33] + 2*fPtr[RHO]*v3*v3*v3;
...
// total heat flux tensor equations
        ...
        double v2 = fPtr[RHOUY]/fPtr[RHO];
        double v3 = fPtr[RHOUZ]/fPtr[RHO];
        fPtr[Q111] = qTen[10*n+0] + 3*v1*fPtr[P11] - 2*fPtr[RHO]*v1*v1*v1;
        fPtr[Q112] = qTen[10*n+1] + 2*v1*fPtr[P12] + v2*fPtr[P11] - 2*fPtr[RHO]*v1*v1*v2;
        fPtr[Q113] = qTen[10*n+2] + 2*v1*fPtr[P13] + v3*fPtr[P11] - 2*fPtr[RHO]*v1*v1*v3;
        fPtr[Q122] = qTen[10*n+3] + v1*fPtr[P22] + 2*v2*fPtr[P12] - 2*fPtr[RHO]*v1*v2*v2;
        fPtr[Q123] = qTen[10*n+4] + v1*fPtr[P23] + v2*fPtr[P13] + v3*fPtr[P12] - 2*fPtr[RHO]*v1*v2*v3;
        fPtr[Q133] = qTen[10*n+5] + v1*fPtr[P33] + 2*v3*fPtr[P13] - 2*fPtr[RHO]*v1*v3*v3;
        fPtr[Q222] = qTen[10*n+6] + 3*v2*fPtr[P22] - 2*fPtr[RHO]*v2*v2*v2;
        fPtr[Q223] = qTen[10*n+7] + 2*v2*fPtr[P23] + v3*fPtr[P22] - 2*fPtr[RHO]*v2*v2*v3;
        fPtr[Q233] = qTen[10*n+8] + v2*fPtr[P33] + 2*v3*fPtr[P23] - 2*fPtr[RHO]*v2*v3*v3;
        fPtr[Q333] = qTen[10*n+9] + 3*v3*fPtr[P33] - 2*fPtr[RHO]*v3*v3*v3;
      }
...
// update electric field
  ...
  template <unsigned NDIM>
  void
  ImplicitTwentyMomentSrcUpdater<NDIM>::declareTypes()
  {
    this->setLastOutVarType(typeid(Lucee::Field<NDIM, double>));
  }

// instantiations
  template class ImplicitTwentyMomentSrcUpdater<1>;
  template class ImplicitTwentyMomentSrcUpdater<2>;
  template class ImplicitTwentyMomentSrcUpdater<3>;
}
