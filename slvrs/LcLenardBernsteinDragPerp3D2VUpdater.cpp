/**
 * @file	LcLenardBernsteinDragPerp3D2VUpdater.cpp
 *
...
// lucee includes
#include <LcLenardBernsteinDragPerp3D2VUpdater.h>
#include <LcGlobals.h>
...
// set id for module system
  const char *LenardBernsteinDragPerp3D2VUpdater::id = "LenardBernsteinDragPerp3D2VUpdater";

  LenardBernsteinDragPerp3D2VUpdater::LenardBernsteinDragPerp3D2VUpdater()
    : UpdaterIfc()
  {
  }

  void 
  LenardBernsteinDragPerp3D2VUpdater::readInput(Lucee::LuaTable& tbl)
  {
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<5> >("basis5d"))
      ...
      throw Lucee::Except("LenardBernsteinDragPerp3D2VUpdater::readInput: Must specify element to use using 'basis5d'");
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<3> >("basis3d"))
      ...
      throw Lucee::Except("LenardBernsteinDragPerp3D2VUpdater::readInput: Must specify element to use using 'basis3d'");
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<2> >("basis2d"))
      ...
      throw Lucee::Except("LenardBernsteinDragPerp3D2VUpdater::readInput: Must specify element to use using 'basis2d'");
    ...
    if (tbl.hasFunction("alpha"))
      ...
      throw Lucee::Except("LenardBernsteinDragPerp3D2VUpdater::readInput: Must supply a collision frequency function as alpha.");
  }

  void 
  LenardBernsteinDragPerp3D2VUpdater::initialize()
  {
  ...
  Lucee::UpdaterStatus 
  LenardBernsteinDragPerp3D2VUpdater::update(double t)
  {
    const Lucee::StructuredGridBase<5>& grid
      = this->getGrid<Lucee::StructuredGridBase<5> >();
            ...
            for (int iv = localRgn.getLower(3); iv < localRgn.getUpper(3); iv++)
            {
              ...
              for (int iMu = localRgn.getLower(4); iMu < localRgn.getUpper(4); iMu++)
              {
                ...
                for (int quadIndex = 0; quadIndex < gaussSurfWeights2d[1].size(); quadIndex++)
                {
                  ...
                  surfIntegralFluxes(quadIndex) = gaussSurfWeights2d[1][quadIndex]*numFlux;
                }
  ...
  void
  LenardBernsteinDragPerp3D2VUpdater::declareTypes()
  {
  ...
  void
  LenardBernsteinDragPerp3D2VUpdater::copyLuceeToEigen(const Lucee::Matrix<double>& sourceMatrix,
  ...
  void
  LenardBernsteinDragPerp3D2VUpdater::evaluateFunction(Lucee::LuaState& L, double tm,
    std::vector<double>& res)
  {
    ...
    {
      Lucee::Except lce("LenardBernsteinDragPerp3D2VUpdater::evaluateFunction: ");
    ...
    {
      if (!lua_isnumber(L, i))
        throw Lucee::Except("LenardBernsteinDragPerp3D2VUpdater::evaluateFunction: Return value not a number");
  ...
  bool
  LenardBernsteinDragPerp3D2VUpdater::sameConfigCoords(int srcIndex, int tarIndex, double dxMin,
