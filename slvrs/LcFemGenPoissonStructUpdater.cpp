/**
 * @file	LcFemGenPoissonStructUpdater.cpp
 *
...
namespace Lucee
{
  ...
  template <> const char *FemGenPoissonStructUpdater<1>::id = "FemGenPoisson1D";
  template <> const char *FemGenPoissonStructUpdater<2>::id = "FemGenPoisson2D";
...
// update() method is called at least once.
  ...
  template <unsigned NDIM>
  void
  FemGenPoissonStructUpdater<NDIM>::readInput(Lucee::LuaTable& tbl)
  {
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<NDIM> >("basis"))
      ...
      throw Lucee::Except(
        "FemGenPoissonStructUpdater::readInput: Must specify element to use using 'basis'");
...
// check if any direction is periodic
    if (tbl.hasNumVec("periodicDirs"))
    {
      ...
      for (unsigned i=0; i<pd.size(); ++i)
      {
        if ((pd[i]<0) || (pd[i]>=NDIM))
        {
          throw Lucee::Except("FemGenPoissonStructUpdater:readInput: Incorrect 'periodicDirs'");
        }
...
// ensure both BCs are specified
        ...
        {
          Lucee::Except lce("FemGenPoissonStructUpdater:: Must specify BCs on each side");
          throw lce;
        }
      }
      else
...
// ensure no BCs are specified
        if (bc[dir][0].isSet || bc[dir][1].isSet)
        {
          Lucee::Except lce("FemGenPoissonStructUpdater:: Cannot specify BCs");
...
// check if stiffness matrix should be written out
  ...
  template <unsigned NDIM>
  void
  FemGenPoissonStructUpdater<NDIM>::initialize()
  {
...
// Setup stuff to compute density-weighted stiffness matrices
...
// diff matrices are computed from transposed stiffness matrices
  ...
  template <unsigned NDIM>      
  void
  FemGenPoissonStructUpdater<NDIM>::calcLocalStiffMatrix(Lucee::Matrix<double>& localStiff,
...
// assume final dimension is the perpendicular direction    
    unsigned DMAX = NDIM;
    if (isGkPoisson) DMAX = NDIM-1;

    unsigned nVolQuad = nodalBasis->getNumGaussNodes();
    std::vector<double> numDensQuad(nVolQuad);
// interpolate density to quadrature nodes
    matVec(1.0, volQuad.interpMat.m, &numDensPtr[0], 0.0, &numDensQuad[0]);
    
    for (unsigned i=0; i<localStiff.numRows(); ++i)
      for (unsigned j=0; j<localStiff.numColumns(); ++j)
      {
        localStiff(i,j) = 0.0;
        for (unsigned k=0; k<nVolQuad; ++k)
        {
// compute inner product 
          double gradgrad = 0.0;
          for (unsigned d=0; d<DMAX; ++d)
            gradgrad += mGradPhi[d].m(i,k)*mGradPhi[d].m(j,k);
...
// construct message to send back to Lua
    ...
    {
      status = false;
      msgStrm << FemGenPoissonStructUpdater<NDIM>::id << ": KSPSolve failed!";
      msgStrm << " Petsc reason code was " << reason << ".";
    }
    else
    {
      msgStrm << FemGenPoissonStructUpdater<NDIM>::id << ": KSPSolve converged.";
    }
...
// copy solution from PetSc array to solution field
  ...
  template <unsigned NDIM>
  void
  FemGenPoissonStructUpdater<NDIM>::declareTypes()
  {
  ...
  template <unsigned NDIM>
  typename FemGenPoissonStructUpdater<NDIM>::FemPoissonBcData
  FemGenPoissonStructUpdater<NDIM>::getBcData(const Lucee::LuaTable& bct) const
  {
    ...
    {
      Lucee::Except lce(
        "FemGenPoissonStructUpdater::readInput: Must specify one of \"D\", \"D_VAR\", or \"N\". ");
  ...
  template <unsigned NDIM>
  double
  FemGenPoissonStructUpdater<NDIM>::getFieldIntegral(const Lucee::Field<NDIM, double>& fld, 
    bool shareFlag)
  {
    unsigned nlocal = nodalBasis->getNumNodes();
...
// get hold of comm pointer to do all parallel messaging
  ...
  template <unsigned NDIM>
  void
  FemGenPoissonStructUpdater<NDIM>::copyFromPetscField(Vec ptFld, Lucee::Field<NDIM, double>& gkFld)
  {
...
// copy data over to Gkeyll field
  ...
  template <unsigned NDIM>
  void
  FemGenPoissonStructUpdater<NDIM>::copyFromGkeyllField(const Lucee::Field<NDIM, double>& gkFld, Vec ptFld)
  {
    PetscScalar *ptFldPtr;
    if (solNodesShared)
    {
...
// already local and does not require communication.)
  ...
  template <unsigned NDIM>  
  void
  FemGenPoissonStructUpdater<NDIM>::matVec(double m, const Lucee::Matrix<double>& mat,
...
// instantiations
  template class FemGenPoissonStructUpdater<1>;
  template class FemGenPoissonStructUpdater<2>;
  template class FemGenPoissonStructUpdater<3>;
}
