/**
 * @file	LcElectromagneticMomentsAtEdgesUpdater.cpp
 *
...
namespace Lucee
{
  const char *ElectromagneticMomentsAtEdgesUpdater::id = "ElectromagneticMomentsAtEdgesUpdater";

  ElectromagneticMomentsAtEdgesUpdater::ElectromagneticMomentsAtEdgesUpdater()
  {
  }

  void
  ElectromagneticMomentsAtEdgesUpdater::readInput(Lucee::LuaTable& tbl)
  {
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<2> >("basis"))
      ...
      throw Lucee::Except("ElectromagneticMomentsAtEdgesUpdater::readInput: Must specify element to use using 'basis'");

    if (tbl.hasNumber("speciesMass"))
      speciesMass = tbl.getNumber("speciesMass");
    else
      throw Lucee::Except("ElectromagneticMomentsAtEdgesUpdater::readInput: Must specify speciesMass");
    
    if (tbl.hasNumber("speciesCharge"))
      speciesCharge = tbl.getNumber("speciesCharge");
    else
      throw Lucee::Except("ElectromagneticMomentsAtEdgesUpdater::readInput: Must specify speciesCharge");
  }

  void
  ElectromagneticMomentsAtEdgesUpdater::initialize()
  {
  ...
  Lucee::UpdaterStatus
  ElectromagneticMomentsAtEdgesUpdater::update(double t)
  {
    ...
    // Output velocity moments (0-3) vs time
    ...
    aIn.setPtr(aPtr, ix);
    // Value of A at the rightmost edge is found at the last node in this cell
    ...
    for (int js = globalRgn.getUpper(1)-1; js >= 0; js--)
    {
      ...
      for (int quadNodeIndex = 0; quadNodeIndex < edgeQuadData.rows(); quadNodeIndex++)
      {
        ...
        mom0InEntireCell += gaussEdgeWeights[quadNodeIndex]*edgeQuadData(quadNodeIndex)/speciesMass;

        mom1InEntireCell += gaussEdgeWeights[quadNodeIndex]*
          (physicalP-speciesCharge*aAtEdge)/(speciesMass*speciesMass)*edgeQuadData(quadNodeIndex);

        mom2InEntireCell += gaussEdgeWeights[quadNodeIndex]*
          2*edgeHamilQuadData[quadNodeIndex]/(speciesMass*speciesMass)*edgeQuadData[quadNodeIndex];

        mom3InEntireCell += gaussEdgeWeights[quadNodeIndex]*2*edgeHamilQuadData[quadNodeIndex]*
          edgeHamilDerivQuadData[quadNodeIndex]/(speciesMass*speciesMass)*edgeQuadData[quadNodeIndex];
      }
    ...
    aIn.setPtr(aPtr, ix);
    // Value of A at the leftmost edge is found at the first node in this cell
    aAtEdge = aPtr[0];
  ...
  void
  ElectromagneticMomentsAtEdgesUpdater::declareTypes()
  {
    // A distribution function
    this->appendOutVarType(typeid(Lucee::Field<2, double>));
    // A hamiltonian (kinetic portion only)
    this->appendOutVarType(typeid(Lucee::Field<2, double>));
    // Vector potential (A)
    this->appendOutVarType(typeid(Lucee::Field<1, double>));
    // Moments 0 to 3 at left and right edges
    this->appendOutVarType(typeid(Lucee::DynVector<double>));
  }

  void
  ElectromagneticMomentsAtEdgesUpdater::copyLuceeToEigen(const Lucee::Matrix<double>& sourceMatrix,
