/**
 * @file	LcPonderomotivePotentialUpdater.cpp
 *
 * @brief	Updater to compute the ponderomotive potential correction
 */
...
// set id for module system
  const char *PonderomotivePotentialUpdater::id = "PonderomotivePotentialUpdater";

  PonderomotivePotentialUpdater::PonderomotivePotentialUpdater()
    : UpdaterIfc()
  {
  }

  void 
  PonderomotivePotentialUpdater::readInput(Lucee::LuaTable& tbl)
  {
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<1> >("basis"))
      ...
      throw Lucee::Except("PonderomotivePotentialUpdater::readInput: Must specify element to use using 'basis'");

    if (tbl.hasNumber("kPerpTimesRho"))
      kPerpTimesRho = tbl.getNumber("kPerpTimesRho");
    else
      throw Lucee::Except("PonderomotivePotentialUpdater::readInput: Must specify kPerpTimesRho");

    if (tbl.hasNumber("Te0"))
      Te0 = tbl.getNumber("Te0");
    else
      throw Lucee::Except("PonderomotivePotentialUpdater::readInput: Must specify Te0");

    if (tbl.hasNumber("ionMass"))
      ionMass = tbl.getNumber("ionMass");
    else
      throw Lucee::Except("PonderomotivePotentialUpdater::readInput: Must specify ionMass");
  }

  void 
  PonderomotivePotentialUpdater::initialize()
  {
  ...
  Lucee::UpdaterStatus 
  PonderomotivePotentialUpdater::update(double t)
  {
    const Lucee::StructuredGridBase<1>& grid
      = this->getGrid<Lucee::StructuredGridBase<1> >();
  ...
  void
  PonderomotivePotentialUpdater::declareTypes()
  {
  ...
  void
  PonderomotivePotentialUpdater::copyLuceeToEigen(const Lucee::Matrix<double>& sourceMatrix,
