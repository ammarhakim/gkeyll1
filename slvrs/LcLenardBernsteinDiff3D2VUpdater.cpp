/**
 * @file	LcLenardBernsteinDiff3D2VUpdater.cpp
 *
...
namespace Lucee
{
  ...
  const char *LenardBernsteinDiff3D2VUpdater::id = "LenardBernsteinDiff3D2VUpdater";

  LenardBernsteinDiff3D2VUpdater::LenardBernsteinDiff3D2VUpdater()
  {
  }
  ...
  void
  LenardBernsteinDiff3D2VUpdater::readInput(Lucee::LuaTable& tbl)
  {
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<5> >("basis5d"))
      ...
      throw Lucee::Except("LenardBernsteinDiff3D2VUpdater::readInput: Must specify element to use using 'basis5d'");
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<3> >("basis3d"))
      ...
      throw Lucee::Except("LenardBernsteinDiff3D2VUpdater::readInput: Must specify element to use using 'basis3d'");
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<2> >("basis2d"))
      ...
      throw Lucee::Except("LenardBernsteinDiff3D2VUpdater::readInput: Must specify element to use using 'basis2d'");
    ...
    if (tbl.hasNumber("speciesMass"))
      ...
      throw Lucee::Except("LenardBernsteinDiff3D2VUpdater::readInput: Must specify speciesMass");

    if (tbl.hasFunction("alpha"))
      fnRef = tbl.getFunctionRef("alpha");
    else
      throw Lucee::Except("LenardBernsteinDiff3D2VUpdater::readInput: Must supply a collision frequency function as alpha.");
  }

  void
  LenardBernsteinDiff3D2VUpdater::initialize()
  {
...
// get hold of grid
  ...
  Lucee::UpdaterStatus
  LenardBernsteinDiff3D2VUpdater::update(double t)
  {
    const Lucee::StructuredGridBase<5>& grid 
      = this->getGrid<Lucee::StructuredGridBase<5> >();
  ...
  void
  LenardBernsteinDiff3D2VUpdater::declareTypes()
  {
  ...
  void
  LenardBernsteinDiff3D2VUpdater::copyLuceeToEigen(const Lucee::Matrix<double>& sourceMatrix,
  ...
  void
  LenardBernsteinDiff3D2VUpdater::evaluateFunction(Lucee::LuaState& L, double tm,
    std::vector<double>& res)
  {
    ...
    {
      Lucee::Except lce("LenardBernsteinDiff3D2VUpdater::evaluateFunction: ");
    ...
    {
      if (!lua_isnumber(L, i))
        throw Lucee::Except("LenardBernsteinDiff3D2VUpdater::evaluateFunction: Return value not a number");
  ...
  bool
  LenardBernsteinDiff3D2VUpdater::sameConfigCoords(int srcIndex, int tarIndex, double dxMin,
