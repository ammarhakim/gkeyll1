/**
 * @file	LcLenardBernsteinDiffPerp3D2VUpdater.cpp
 *
...
// lucee includes
#include <LcLenardBernsteinDiffPerp3D2VUpdater.h>
#include <LcGlobals.h>
...
namespace Lucee
{
  ...
  const char *LenardBernsteinDiffPerp3D2VUpdater::id = "LenardBernsteinDiffPerp3D2VUpdater";

  LenardBernsteinDiffPerp3D2VUpdater::LenardBernsteinDiffPerp3D2VUpdater()
  {
  }
  ...
  void
  LenardBernsteinDiffPerp3D2VUpdater::readInput(Lucee::LuaTable& tbl)
  {
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<5> >("basis5d"))
      ...
      throw Lucee::Except("LenardBernsteinDiffPerp3D2VUpdater::readInput: Must specify element to use using 'basis5d'");
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<3> >("basis3d"))
      ...
      throw Lucee::Except("LenardBernsteinDiffPerp3D2VUpdater::readInput: Must specify element to use using 'basis3d'");
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<2> >("basis2d"))
      ...
      throw Lucee::Except("LenardBernsteinDiffPerp3D2VUpdater::readInput: Must specify element to use using 'basis2d'");
    ...
    if (tbl.hasNumber("speciesMass"))
      ...
      throw Lucee::Except("LenardBernsteinDiffPerp3D2VUpdater::readInput: Must specify speciesMass");

    if (tbl.hasFunction("alpha"))
      fnRef = tbl.getFunctionRef("alpha");
    else
      throw Lucee::Except("LenardBernsteinDiffPerp3D2VUpdater::readInput: Must supply a collision frequency function as alpha.");
  }

  void
  LenardBernsteinDiffPerp3D2VUpdater::initialize()
  {
...
// get hold of grid
  ...
  Lucee::UpdaterStatus
  LenardBernsteinDiffPerp3D2VUpdater::update(double t)
  {
    const Lucee::StructuredGridBase<5>& grid 
      = this->getGrid<Lucee::StructuredGridBase<5> >();
          ...
          for (int configNode = 0; configNode < nlocal3d; configNode++)
          {
            ...
            for (int iv = localRgn.getLower(3); iv < localRgn.getUpper(3); iv++)
            {
              ...
              for (int iMu = localRgn.getLower(4); iMu < localRgn.getUpper(4); iMu++)
              {
                ...
                Eigen::VectorXd updateF = (2*speciesMass/bFieldInPtr[configNode]*(cellCentroid[4]*selfCenter[1] + selfCenterTimesMu) )*fReduced;
  ...
  void
  LenardBernsteinDiffPerp3D2VUpdater::declareTypes()
  {
  ...
  void
  LenardBernsteinDiffPerp3D2VUpdater::copyLuceeToEigen(const Lucee::Matrix<double>& sourceMatrix,
  ...
  void
  LenardBernsteinDiffPerp3D2VUpdater::evaluateFunction(Lucee::LuaState& L, double tm,
    std::vector<double>& res)
  {
    ...
    {
      Lucee::Except lce("LenardBernsteinDiffPerp3D2VUpdater::evaluateFunction: ");
    ...
    {
      if (!lua_isnumber(L, i))
        throw Lucee::Except("LenardBernsteinDiffPerp3D2VUpdater::evaluateFunction: Return value not a number");
  ...
  bool
  LenardBernsteinDiffPerp3D2VUpdater::sameConfigCoords(int srcIndex, int tarIndex, double dxMin,
