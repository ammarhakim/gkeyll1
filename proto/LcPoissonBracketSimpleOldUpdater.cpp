/**
 * @file	LcPoissonBracketSimpleOldUpdater.cpp
 *
 * @brief	Updater to solver Poisson bracket operator PDEs.
 * This version assumes there is no transformation jacobian, allowing
 * for some speedups (?).
 *
 * ELS 11/12/11: This is the orignal version of LcPoissonBracketSimpleUpdater that works
 * for the 1D2V SOL code. The current version of LcPoissonBracketSimpleUpdater does not work for 1D2V SOL.
 */
...
// lucee includes
#include <LcMathLib.h>
#include <LcPoissonBracketSimpleOldUpdater.h>
...
// set id for module system
  template <> const char *PoissonBracketSimpleOldUpdater<1>::id = "PoissonBracketSimpleOld1D";
  template <> const char *PoissonBracketSimpleOldUpdater<2>::id = "PoissonBracketSimpleOld2D";
  template <> const char *PoissonBracketSimpleOldUpdater<3>::id = "PoissonBracketSimpleOld3D";
  template <> const char *PoissonBracketSimpleOldUpdater<4>::id = "PoissonBracketSimpleOld4D";
  template <> const char *PoissonBracketSimpleOldUpdater<5>::id = "PoissonBracketSimpleOld5D";

  template <unsigned NDIM>
  PoissonBracketSimpleOldUpdater<NDIM>::PoissonBracketSimpleOldUpdater()
    : UpdaterIfc()
  {
  }
  ...
  template <unsigned NDIM>
  void 
  PoissonBracketSimpleOldUpdater<NDIM>::readInput(Lucee::LuaTable& tbl)
  {
// call base class method
    ...
    if (tbl.hasObject<Lucee::NodalFiniteElementIfc<NDIM> >("basis"))
      ...
      throw Lucee::Except("PoissonBracketSimpleOldUpdater::readInput: Must specify element to use using 'basis'");

    if (tbl.hasObject<Lucee::PoissonBracketEquation>("equation"))
      equation = &tbl.getObjectAsBase<Lucee::PoissonBracketEquation>("equation");
    else
    {
      Lucee::Except lce("PoissonBracketSimpleOldUpdater::readInput: Must specify an equation to solve!");
      throw lce;
    }
    ...
    if (tbl.hasString("fluxType"))
    {
      ...
      {
        Lucee::Except lce("PoissonBracketSimpleOldUpdater::readInput: 'fluxType' ");
    ...
    if (tbl.hasNumVec("updateDirections"))
    {
      ...
      for (int i = 0; i < std::min<int>(NDIM, tempDirs.size()); i++)
      {
        ...
        if (d < NDIM)
          ...
          throw Lucee::Except("PoissonBracketSimpleOldUpdater::readInput: updateDirections must be a table with each element < NDIM");
      }
    }
    else
    {
    ...
    if (tbl.hasNumVec("zeroFluxDirections"))
    {
      ...
      for (int i = 0; i < std::min<int>(NDIM, tempDirs.size()); i++)
      {
        ...
        if (d < NDIM)
          ...
          throw Lucee::Except("PoissonBracketSimpleOldUpdater::readInput: zeroFluxDirections must be a table with each element < NDIM");
      }
    }
  }
  ...
  template <unsigned NDIM>
  void 
  PoissonBracketSimpleOldUpdater<NDIM>::initialize()
  {
    // call base class method
  ...
  template <unsigned NDIM>
  Lucee::UpdaterStatus 
  PoissonBracketSimpleOldUpdater<NDIM>::update(double t)
  {
    const Lucee::StructuredGridBase<NDIM>& grid 
      = this->getGrid<Lucee::StructuredGridBase<NDIM> >();
  ...
  template <unsigned NDIM>
  void
  PoissonBracketSimpleOldUpdater<NDIM>::declareTypes()
  {
  ...
  template <unsigned NDIM>
  void
  PoissonBracketSimpleOldUpdater<NDIM>::computeNumericalFlux(const Eigen::VectorXd& alphaDotN,
  ...
  template <unsigned NDIM>
  void
  PoissonBracketSimpleOldUpdater<NDIM>::copyLuceeToEigen(const Lucee::Matrix<double>& sourceMatrix,
  ...
  // instantiations
  template class PoissonBracketSimpleOldUpdater<1>;
  template class PoissonBracketSimpleOldUpdater<2>;
  template class PoissonBracketSimpleOldUpdater<3>;
  template class PoissonBracketSimpleOldUpdater<4>;
  template class PoissonBracketSimpleOldUpdater<5>;
}
