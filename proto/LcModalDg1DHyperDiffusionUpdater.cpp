/**
 * @file	LcModalDg1DHyperDiffusionUpdater.cpp
 *
...
/** Class id: this is used by registration system */
  const char *ModalDg1DHyperDiffusionUpdater::id = "ModalDg1DHyperDiffusion";

  ModalDg1DHyperDiffusionUpdater::ModalDg1DHyperDiffusionUpdater()
    : UpdaterIfc()
  {
  }

  void
  ModalDg1DHyperDiffusionUpdater::readInput(Lucee::LuaTable& tbl)
  {
  ...
  void
  ModalDg1DHyperDiffusionUpdater::initialize()
  {
    // Get hold of grid
    ...
    thirdDerivEdgeEvals = Eigen::MatrixXd::Zero(numBasis, 2);
    ...
    for (int polyOrder = 0; polyOrder < numBasis; polyOrder++)
    {
      if (polyOrder < 2)
      {
        ...
        thirdDerivEdgeEvals(polyOrder,0) = 0.0;
        thirdDerivEdgeEvals(polyOrder,1) = 0.0;
      }
      else
      {
        secondDerivEdgeEvals(polyOrder,0) = ((2*polyOrder-1)*(sgn*(polyOrder-1)*polyOrder - 
                                              secondDerivEdgeEvals(polyOrder-1,0)) - 
                                              (polyOrder-1)*secondDerivEdgeEvals(polyOrder-2,0))/polyOrder;
        secondDerivEdgeEvals(polyOrder,1) = ((2*polyOrder-1)*((polyOrder-1)*polyOrder + 
        ...
        {
          thirdDerivEdgeEvals(polyOrder,0) = (2*polyOrder-1)*(3*secondDerivEdgeEvals(polyOrder-1,0) - 
                                              thirdDerivEdgeEvals(polyOrder-1,0) - 
                                              (polyOrder-1)*thirdDerivEdgeEvals(polyOrder-2,0))/polyOrder;
          thirdDerivEdgeEvals(polyOrder,1) = (2*polyOrder-1)*(3*secondDerivEdgeEvals(polyOrder-1,1) + 
                                              thirdDerivEdgeEvals(polyOrder-1,1) - 
                                              (polyOrder-1)*thirdDerivEdgeEvals(polyOrder-2,1))/polyOrder;
        }

        // Scale into physical space integrals
        secondDerivEdgeEvals(polyOrder,0) = 4.0/(dx*dx)*secondDerivEdgeEvals(polyOrder,0);
        secondDerivEdgeEvals(polyOrder,1) = 4.0/(dx*dx)*secondDerivEdgeEvals(polyOrder,1);
        thirdDerivEdgeEvals(polyOrder,0) = 8.0/(dx*dx*dx)*thirdDerivEdgeEvals(polyOrder,0);
        thirdDerivEdgeEvals(polyOrder,1) = 8.0/(dx*dx*dx)*thirdDerivEdgeEvals(polyOrder,1);
      }
...
// call base class method
  ...
  Lucee::UpdaterStatus
  ModalDg1DHyperDiffusionUpdater::update(double t)
  {
...
// time-step
    ...
    if(dt > (cfl*dx*dx*dx*dx/(2*diffCoef)))
    {
      // time-step was too large: return a suggestion with correct time-step
      return Lucee::UpdaterStatus(false, cfl*dx*dx*dx*dx/(2*diffCoef));
    }
...
// state on left and right of each edge
    ...
    {
      Lucee::Except lce(
        "ModalDg1DHyperDiffusionUpdater::update: Number of components in input/output fields should be ");
...
// lower and upper bounds of slice.
    ...
    std::vector<double> boundaryTerms(4);
...
// accumulate increment to appropriate cells
      ...
      {
        qNewlPtr[m] += boundaryTerms[3] - (m*(m+1)/dx)*boundaryTerms[2] + secondDerivEdgeEvals(m,1)*boundaryTerms[1] -
                        thirdDerivEdgeEvals(m,1)*boundaryTerms[0];
        qNewrPtr[m] += sgn*(boundaryTerms[3] + (m*(m+1)/dx)*boundaryTerms[2]) - secondDerivEdgeEvals(m,0)*boundaryTerms[1] +
                        thirdDerivEdgeEvals(m,0)*boundaryTerms[0];
        sgn *= -1;
      }
    }
...
// This will only be needed if numBasis > 2
    ...
    return Lucee::UpdaterStatus(true, cfl*dx*dx*dx*dx/(2*diffCoef));
  }

  void
  ModalDg1DHyperDiffusionUpdater::declareTypes()
  {
    this->appendInpVarType(typeid(Lucee::Field<1, double>));
    this->appendOutVarType(typeid(Lucee::Field<1, double>));
  }

  double ModalDg1DHyperDiffusionUpdater::legendrePoly2ndDeriv(int order, double x)
  {
  ...
  void
  ModalDg1DHyperDiffusionUpdater::computeRecoveryPolynomial(const Lucee::ConstFieldPtr<double>& qLeft,
    Lucee::ConstFieldPtr<double>& qRight, std::vector<double>& boundaryTerms)
  {
    ...
    if (numBasis > 1)
    {
      boundaryTerms[2] = fVals(2,0);
      boundaryTerms[3] = fVals(3,0);
    }
    else
    {
      boundaryTerms[2] = 0.0;
