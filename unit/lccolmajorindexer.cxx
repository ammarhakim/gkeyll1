...
test_1()
{
  int start[1] = {1};
  unsigned shape[1] = {10};
  Lucee::ColMajorIndexer<1> col(start, shape);

  int count=0;
  for (int i=col.getStart(0); i<col.getEnd(0); ++i)
...
test_2()
{
  ...
  Lucee::ColMajorIndexer<2> col(start, shape);

  int count=0;
  for (int j=col.getStart(1); j<col.getEnd(1); ++j)
    for (int i=col.getStart(0); i<col.getEnd(0); ++i)
...
test_3()
{
  ...
  Lucee::ColMajorIndexer<3> col(start, shape);

  int count=0;
  for (int k=col.getStart(2); k<col.getEnd(2); ++k)
    for (int j=col.getStart(1); j<col.getEnd(1); ++j)
      for (int i=col.getStart(0); i<col.getEnd(0); ++i)
...
test_4()
{
  ...
  Lucee::ColMajorIndexer<4> col(start, shape);

  int count=0;
  for (int l=col.getStart(3); l<col.getEnd(3); ++l)
    for (int k=col.getStart(2); k<col.getEnd(2); ++k)
      for (int j=col.getStart(1); j<col.getEnd(1); ++j)
        for (int i=col.getStart(0); i<col.getEnd(0); ++i)
