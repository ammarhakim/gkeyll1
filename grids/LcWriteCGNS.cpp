...
WriterIface *LcWriteCGNS::factory( Interface* iface )
  { return new LcWriteCGNS( iface ); }

LcWriteCGNS::LcWriteCGNS(Interface *impl)
    : mbImpl(impl), VrtSize(0), EdgeSize(0), FaceSize(0), CellSize(0)
{
  impl->query_interface(mWriteIface);
}

LcWriteCGNS::~LcWriteCGNS()
{
  mbImpl->release_interface(mWriteIface);
}

//! writes out a file
ErrorCode LcWriteCGNS::write_file(const char *file_name,
...
{
  ErrorCode rval;
  std::cout << "Something really has changed\n";
  std::cout << "Attempting to write CGNS file\n";
  ...
  isize[1] = CellSize;
  if(physdim==2) {//JLO did this, makes sense right?
    isize[1] = FaceSize;  // isize[1] contains the total cell size
  }
  ...
  std::cout << "The number of dimensions is " << physdim << "\n";
  ...
  switch ( physdim ){
    ...
    case 2:
{
      std::cout << "We should be writing set 2!\n";
      ...
      for (int i = 0; i < NbTags; ++i) {
        std::cout << "will try and write set " << i << "\n";
        std::cout << "ConnTable.size() " << ConnTable[i].size() << "\n";
        if ( Sets[i].IdSet != -1 ){
            std::cout << "Do any sets have a non-zero Id?\n";
            const char * SectionName = Sets[i].TagName.c_str();
      ...
      //JLO put this here -- default case

      std::vector<EntityHandle> thisConn;    // Array in which connectivity of entity_handle is returned.
      // Gets a pointer to constant connectivity data of entity_handle
      rval = mbImpl->get_connectivity_by_type( MBTRI, thisConn);

      std::vector<int> newConn(thisConn.size());
      for(int j=0; j<thisConn.size(); j++) {
        newConn[j]=thisConn[j];
      }

      EndSetsIndex = BeginSetsIndex + Edges.size() + Faces.size() - 1;
      if ( cg_section_write( IndexFile, IndexBase, IndexBase, "Elements", TRI_3,
                                   BeginSetsIndex, EndSetsIndex, 0, &newConn[0],&IndexSection) ){
            std::cout << "Issue on writing connectivity -- 2-D\n";
            cg_error_exit();
          }
}
      break;
    case 3:
      std::cout << "We should be writing set 3!\n";
...
// Get and count vertex entities
ErrorCode LcWriteCGNS::get_vertex_entities ( cgsize_t &VrtSize_, std::vector< moab::EntityHandle > &Nodes_)
{
   ErrorCode rval;
   // Get vertex entities
...
// Get and count edge entities
ErrorCode LcWriteCGNS::get_edge_entities(cgsize_t &EdgeSize_, std::vector< moab::EntityHandle > &Edges_)
{
  ErrorCode rval;
  // The first input is 0 because one queries the entire mesh.
...
// Get and count face entities
ErrorCode LcWriteCGNS::get_face_entities(cgsize_t &FaceSize_, std::vector< moab::EntityHandle > &Faces_)
{
  ErrorCode rval;
  // The first input is 0 because one queries the entire mesh.
...
// Get and count cell entities
ErrorCode LcWriteCGNS::get_cell_entities(cgsize_t &CellSize_, std::vector< moab::EntityHandle > &Cells_)
{
  ErrorCode rval;
  // The first input is 0 because one queries the entire mesh.
...
ErrorCode LcWriteCGNS::write_coord_cgns(std::vector< moab::EntityHandle > &Nodes_)
{
  ErrorCode rval;
...
ErrorCode LcWriteCGNS::get_set_entities(int i, std::vector< Tag >& TagHandles,
              std::vector< LcWriteCGNS::SetStruct >& Sets)
{
  ErrorCode rval;
...
// Get CGNSType
ErrorCode LcWriteCGNS::get_cgns_type ( int i, std::vector<LcWriteCGNS::SetStruct> &Sets )
{
  std::vector<int> Test;
  int Sum=0;
  ...
  std::cout << "Type BAR_2 " << BAR_2 << " Type Tri_3 " << TRI_3 << " Type Null " << ElementTypeNull << "\n";
...
ErrorCode LcWriteCGNS::get_conn_table( std::vector< moab::EntityHandle > &Elements,
...
int LcWriteCGNS::moab_cgns_conv(const EntityHandle handle)
{
