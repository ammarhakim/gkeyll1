...
namespace Lucee
{
  ...
  class UnstructGridField: public Lucee::Field<NDIM, T>
  {
    public:
       ...
       */
      UnstructGridField(Lucee::UnstructuredGrid<NDIM>* grid, unsigned nc);
       ...
       */
      UnstructGridField<NDIM, T>& operator=(
       ...
       */
      void setGhostFromLuaFunction(lua_State *L, int ref, unsigned dir,
          unsigned side);

    private:
      /** Pointer to grid */
      Lucee::UnstructuredGrid<NDIM> *grid;
       ...
       */
      UnstructGridField(const Field<NDIM, T>& fld,
