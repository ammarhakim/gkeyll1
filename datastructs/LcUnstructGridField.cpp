...
// names used in registration system
  template <> const char *UnstructGridField<1, double>::id = "Field1D";
  template <> const char *UnstructGridField<2, double>::id = "Field2D";
  template <> const char *UnstructGridField<3, double>::id = "Field3D";
  template <> const char *UnstructGridField<4, double>::id = "Field4D";
  template <> const char *UnstructGridField<5, double>::id = "Field5D";
  ...
  template <unsigned NDIM, typename T>
  UnstructGridField<NDIM, T>::UnstructGridField(Lucee::StructuredGridBase<NDIM>* grid, unsigned nc,
  ...
  template <unsigned NDIM, typename T>
  UnstructGridField<NDIM, T>::UnstructGridField(const UnstructGridField<NDIM, T>& fld)
...
// call base class assignment operator
  ...
  template <unsigned NDIM, typename T>
  void
  UnstructGridField<NDIM, T>::divergence(Lucee::UnstructGridField<NDIM, T>& div) const
  {
// WARNING: THIS CODE PRESENTLY ONLY WORKS FOR RECTCART GRIDS

    if (this->getNumComponents() != NDIM)
    {
      Lucee::Except lce("UnstructGridField::divergence: Incorrect number of components. Should be ");
...
// create various iterators
  ...
  template <unsigned NDIM, typename T>
  void
  UnstructGridField<NDIM, T>::readInput(Lucee::LuaTable& tbl)
  {
// get name of grid on which field lives
    if (tbl.template hasObject<Lucee::StructuredGridBase<NDIM> >("onGrid"))
      ...
      throw Lucee::Except("UnstructGridField::readInput: must specify 'onGrid', the grid on which field lives");
...
// read number of ghost cells
    if (tbl.hasNumVec("ghost"))
    {
      ...
      if (gstDbl.size() != 2)
        throw Lucee::Except("UnstructGridField::readInput: must specify exactly two entries in 'ghost' table");
      for (unsigned i=0; i<NDIM; ++i)
      {
...
// read number of ghost cells to write
    if (tbl.hasNumVec("writeGhost"))
    {
      ...
      if (gstDbl.size() != 2)
        throw Lucee::Except("UnstructGridField::readInput: must specify exactly two entries in 'writeGhost' table");
      for (unsigned i=0; i<NDIM; ++i)
      {
...
// create new field and copy data to self
  ...
  template <unsigned NDIM, typename T>
  TxIoNodeType
  UnstructGridField<NDIM, T>::writeToFile(TxIoBase& io, TxIoNodeType& node, const std::string& nm)
  {
// first write the field data to file
    TxIoNodeType dn 
      = Lucee::Field<NDIM, T>::writeToFileWithGhost(io, node, 
        lowerWriteGhost, upperWriteGhost, "UnstructGridField");
...
// now write out grid
  ...
  template <unsigned NDIM, typename T>
  void
  UnstructGridField<NDIM, T>::writeToTxtFile(std::ofstream& txtFl)
  {
...
// now write out actual data at this location
  ...
  template <unsigned NDIM, typename T>
  TxIoNodeType
  UnstructGridField<NDIM, T>::readFromFile(TxIoBase& io, TxIoNodeType& node,
    const std::string& nm)
  {
    TxIoNodeType dn 
      = Lucee::Field<NDIM, T>::readFromFile(io, node, "UnstructGridField");
    return dn;
  }

  template <unsigned NDIM, typename T>
  void
  UnstructGridField<NDIM, T>::sync()
  {
...
// finish receiving data as recieve was non-blocking
  ...
  template <unsigned NDIM, typename T>
  void
  UnstructGridField<NDIM, T>::startRecv(unsigned rank, int lg[NDIM], int ug[NDIM])
  {
...
//  set up a communication
  ...
  template <unsigned NDIM, typename T>
  void
  UnstructGridField<NDIM, T>::send(unsigned rank, int lg[NDIM], int ug[NDIM])
  {
// get hold of comm pointer to do all parallel messaging
    TxCommBase *comm = this->getComm();
...
// -1
  ...
  template <unsigned NDIM, typename T>
  void
  UnstructGridField<NDIM, T>::finishRecv()
  {
...
// ordering of data as we use below.
  ...
  template <unsigned NDIM, typename T>
  void
  UnstructGridField<NDIM, T>::appendLuaCallableMethods(Lucee::LuaFuncMap& lfm)
  {
...
// now append local methods
  ...
  template <unsigned NDIM, typename T>
  int
  UnstructGridField<NDIM, T>::luaSet(lua_State *L)
  {
    ...
    {
      Lucee::Except lce("UnstructGridField::luaSet: Must provide a Lua function to 'set' method");
      throw lce;
    }
  ...
  template <unsigned NDIM, typename T>
  int
  UnstructGridField<NDIM, T>::luaSetGhost(lua_State *L)
  {
    UnstructGridField<NDIM, T> *sgf
      = Lucee::PointerHolder<UnstructGridField<NDIM, T> >::getObjAsDerived(L);
...
// determine direction in which to apply function BCs
    ...
    if (! lua_isstring(L, 4))
    {
      Lucee::Except lce("UnstructGridField::luaFuncBc: Must provide a side to 'applyFuncBc' method.");
...
// determine side in which to apply periodic BCs
    ...
    if (ss == "lower")
      ...
      throw Lucee::Except("UnstructGridField::luaFuncBc: side should be one of \"lower\" or \"upper\".");
  ...
  template <unsigned NDIM, typename T>
  int
  UnstructGridField<NDIM, T>::luaAlias(lua_State *L)
  {
    UnstructGridField<NDIM, T> *sgf
      = Lucee::PointerHolder<UnstructGridField<NDIM, T> >::getObjAsDerived(L);
...
// copy data for ghost I/O
  ...
  template <unsigned NDIM, typename T>
  int
  UnstructGridField<NDIM, T>::luaDuplicate(lua_State *L)
  {
    UnstructGridField<NDIM, T> *sgf
      = Lucee::PointerHolder<UnstructGridField<NDIM, T> >::getObjAsDerived(L);

    Lucee::UnstructGridField<NDIM, T>* aliasSgf =
      new Lucee::UnstructGridField<NDIM, T>(
  ...
  template <unsigned NDIM, typename T>
  int
  UnstructGridField<NDIM, T>::luaDivergence(lua_State *L)
  {
    UnstructGridField<NDIM, T> *sgf
      = Lucee::PointerHolder<UnstructGridField<NDIM, T> >::getObjAsDerived(L);
    if (lua_type(L, 2) != LUA_TUSERDATA)
    {
      Lucee::Except lce("UnstructGridField::luaDivergence: Must provide a field to 'div' method");
      throw lce;
    }
    Lucee::PointerHolder<UnstructGridField<NDIM, T> > *fldPtr =
      (Lucee::PointerHolder<UnstructGridField<NDIM, T> >*) lua_touserdata(L, 2);
// compute divergence
...
// push variables on stack
      ...
      {
        Lucee::Except lce("UnstructGridField::setFromLuaFunction: ");
...
// fetch results
      ...
      {
        if (!lua_isnumber(L, i))
          throw Lucee::Except("UnstructGridField::setFromLuaFunction: Return value not a number");
...
// push variables on stack
      ...
      if (lua_pcall(L, 3, numOut, 0) != 0)
      {
        Lucee::Except lce("UnstructGridField::setGhostFromLuaFunction: ");
        lce << "Problem evaluating function supplied to 'set' method";
        throw lce;
      }
// fetch results
      ...
      {
        if (!lua_isnumber(L, i))
          throw Lucee::Except("UnstructGridField::setGhostFromLuaFunction: Return value not a number");
        ptr[numOut+i] = lua_tonumber(L, i);
      }
      lua_pop(L, 1);
    }
  ...
  template <unsigned NDIM, typename T>
  UnstructGridField<NDIM, T>::UnstructGridField(const Field<NDIM, T>& fld, Lucee::StructuredGridBase<NDIM>& grd)
...
// instantiations
  ...
  template class UnstructGridField<4, int>;
  template class UnstructGridField<5, int>;

  template class UnstructGridField<1, float>;
  template class UnstructGridField<2, float>;
  template class UnstructGridField<3, float>;
  template class UnstructGridField<4, float>;
  template class UnstructGridField<5, float>;
  ...
  template class UnstructGridField<4, double>;
  template class UnstructGridField<5, double>;
